...
double Eff_L(double x) {
    double a1 = -3.20274e-06; 
    double b1 = 0.000134008;
    double c1 = 0.711443; 
...
void UR(TString filename, TString histname, TCanvas* Sources1) {
    ...
    TF1 *gausFit6 = new TF1("gausFit6", "gaus", 3960, 3987); 
    myhist->Fit(gausFit6, "R"); 
    gausFit6->SetLineColor(20);
    gausFit6->Draw("SAME");
    double N_t6 = gausFit6->Integral(3960, 3987);
    double N_b6 = (myhist->Integral(3933, 3960) + myhist->Integral(3987, 4014)) / 2;
    ...
    TF1 *gausFit7 = new TF1("gausFit7", "gaus", 4185, 4210); 
    myhist->Fit(gausFit7, "R"); 
    gausFit7->SetLineColor(21);
    gausFit7->Draw("SAME");
    double N_t7 = gausFit7->Integral(4185, 4210);
    double N_b7 = (myhist->Integral(4160, 4185) + myhist->Integral(4210, 4235)) / 2;
...
UR("Ur_day3.xy", "Uranium", c1);
TFile *Ur1 = new TFile("Ur_Run3.root", "RECREATE"); 
c1->Write();
Ur1->Close(); 
